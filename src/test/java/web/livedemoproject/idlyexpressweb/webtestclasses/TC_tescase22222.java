...
public class TC_tescase22222 extends BaseClass { 
	...
	public TC_tescase22222() throws Exception {
		configFileObj = new ConfigFilesUtility();
		qf = new QFUtilities();
		configFileObj.loadPropertyFile("tc_tescase22222.properties");
	}
	...
	private void newscreen3Test(int i) throws Exception {

		New_screen-3 newscreen3Obj = new New_screen-3();
		try { 
			qf.clickAction(driver, newscreen3Obj._storesetup, configFileObj.getBooleanProperty("storesetup2"  + i + "click"));
			if(configFileObj.getBooleanProperty("storesetup2"  + i + "click"))
			printSuccessLogAndReport( "Clicked on Store setup");
			qf.clickAction(driver, newscreen3Obj._saveChanges, configFileObj.getBooleanProperty("saveChanges2"  + i + "click"));
			if(configFileObj.getBooleanProperty("saveChanges2"  + i + "click"))
			printSuccessLogAndReport( "Clicked on Save Changes");
			qf.clickAction(driver, newscreen3Obj._saveChanges, configFileObj.getBooleanProperty("saveChanges2"  + i + "click"));
			if(configFileObj.getBooleanProperty("saveChanges2"  + i + "click"))
			printSuccessLogAndReport( "Clicked on Save Changes");
			qf.clickAction(driver, newscreen3Obj._sAVE, configFileObj.getBooleanProperty("sAVE2"  + i + "click"));
			if(configFileObj.getBooleanProperty("sAVE2"  + i + "click"))
			printSuccessLogAndReport( "Clicked on SAVE");
	...
	public void screensTest() throws Exception {
		for(int datasets = 1; datasets <= configFileObj.getIntProperty("datasetsLength"); datasets++) {
			...
			setup();			
			setTestcaseName(browserName,"TC_tescase22222 - " + configFileObj.getProperty("dataset" + (datasets)));
			if(isElementDispalyed) {newscreen2Test(datasets);}
			if(isElementDispalyed) {newscreen3Test(datasets);}
			tearDown();
		}	}
